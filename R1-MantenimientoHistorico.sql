...
CREATE OR REPLACE FUNCTION trg_check_tamanoTallo_not_null()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.tamanoTallo IS NULL THEN
        RAISE EXCEPTION 'El campo tamanoTallo no puede ser NULL.';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Crear el trigger para la tabla HISTORICO_PRECIO_FLOR
CREATE TRIGGER trg_check_tamanoTallo_not_null
BEFORE INSERT ON HISTORICO_PRECIO_FLOR
FOR EACH ROW
EXECUTE FUNCTION trg_check_tamanoTallo_not_null();

----------------------------------------------------------------------
-- Superfuncion del requerimiento 1 --
----------------------------------------------------------------------
